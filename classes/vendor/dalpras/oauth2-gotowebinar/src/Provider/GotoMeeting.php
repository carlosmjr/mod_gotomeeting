...
namespace GoToMeeting\DalPraS\OAuth2\Client\Provider;
...
/**
 * GotoMeeting Provider.
 *
 * @see https://goto-developer.logmeininc.com/content/GotoMeeting-api-reference-v2
 */
class GotoMeeting extends AbstractProvider
{
    ...
    protected function checkResponse(ResponseInterface $response, $data)
    {
        if ($response->getStatusCode() >= 400) {
            throw GotoMeetingProviderException::clientException($response, $data);
        } elseif (isset($data['error'])) {
            throw GotoMeetingProviderException::oauthException($response, $data);
        }
    }
    ...
    {
        $user = new GotoMeetingResourceOwner($response);
