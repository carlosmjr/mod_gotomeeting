...
Usage is the same as The League's OAuth client, using `\GoToMeeting\DalPraS\OAuth2\Client\Provider\GotoWebinar` as the provider.
...
```php
    ...
    $development = true;
    $provider = new \GoToMeeting\DalPraS\OAuth2\Client\Provider\GotoWebinar([
...
```php
    ...
    $resWebinar = new \GoToMeeting\DalPraS\OAuth2\Client\Resources\Webinar($provider, $accessToken);
    try {
        ...
        $dateUtcHelper = new \GoToMeeting\DalPraS\OAuth2\Client\Helper\DateUtcHelper();
