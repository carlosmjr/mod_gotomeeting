...
/**
 * Redirects to the start GoToMeeting url
 *
...
$meetingresponse = $resmeeting->startMeeting($meetingid);

$hascapstartmeeting = has_capability('mod/gotomeeting:start_meeting', $context);
...
if (!empty($meetingresponse) && array_key_exists('hostURL', $meetingresponse) && $hascapstartmeeting) {
    redirect($meetingresponse['hostURL']);
} else {
    redirect(new moodle_url("$CFG->wwwroot/mod/gotomeeting/view.php", array('id' => $cmid )));
}